// Autogenerated by the deployments script
import remoteGoerliPreparetestcomp from "../deployments/remote/remote.goerli.prepare-testcomp.json";
import remotePlymumbaiPreparetestcomp from "../deployments/remote/remote.ply-mumbai.prepare-testcomp.json";
import remotePlymumbaiTestcomp from "../deployments/remote/remote.ply-mumbai.testcomp.json";

export const Deployments = {
    "goerli.prepare-testcomp": remoteGoerliPreparetestcomp,
    "ply-mumbai.prepare-testcomp": remotePlymumbaiPreparetestcomp,
    "ply-mumbai.testcomp": remotePlymumbaiTestcomp,
    ganache: {
        collateralTokenAddress: "0x959922bE3CAee4b8Cd9a407cc3ac1C251C2007B1",
        opynAddressBookAddress: "0x5FbDB2315678afecb367f032d93F642f64180aa3",
        potionLiquidityPoolAddress: "0x59b670e9fA9D0A427751Af201D676719a970857b",
        curveManagerAddress: "0x9A9f2CCfdE556A7E9Ff0848998Aa4a0CFD8863AE",
        criteriaManagerAddress: "0x68B1D87F95878fE05B998F19b66F4baba5De1aed",
        marginVaultLibAddress: "0xa513E6E4b8f2a923D98304ec87F64353C4D5C853",
        otokenFactoryAddress: "0xDc64a140Aa3E981100a9becA4E685f962f0cF6C9",
        whitelistAddress: "0x5FC8d32690cc91D4c39d9d3abcBD16989F875707",
        network: "ganache",
        sampleUnderlyingTokenAddress: "0x322813Fd9A801c5507c9de605d63CEA4f2CE6c44",
        oracleIsMock: true,
    },
    "goerli.independent": {
        timestamp: 1661872235,
        network: "goerli",
        contracts: {
            WETH: {
                address: "0x9889DfADE1d68488590DF17bbA882914535a8F92",
                blockNumber: 7498251,
            },
            WBTC: {
                address: "0x667c04420C2B8D319ac24f6E605dCC28759C55f4",
                blockNumber: 7498257,
            },
            USDC: {
                address: "0x786A7c36d8b3acE2AE2A62c00D915C9f84eaAcB7",
                blockNumber: 7498264,
            },
            ChainlinkAggregatorUSDC: {
                address: "0x5fea417c193828eCF578933121De0B943E356a92",
                blockNumber: 7498271,
            },
            PotionLiquidityPool: {
                address: "0x49f957b11d1c8159a490f3cf4df95110d0630a41",
                blockNumber: 7516604,
            },
            CurveManager: {
                address: "0x809Ce01AD915685878649E5201808af6AE01CDc4",
                blockNumber: 7516589,
            },
            CriteriaManager: {
                address: "0xD77399aEf4d4c7A54eA20eF0943b9e722dEf1339",
                blockNumber: 7516597,
            },
            OtokenFactory: {
                address: "0x41D4Fe19c8C41Dc60638B5d12Cf32d59CFbac73a",
                blockNumber: 7516546,
            },
            Whitelist: {
                address: "0x8a97ef0A5b4AD17DbE509c2fd0F68879aa9904ad",
                blockNumber: 7516553,
            },
        },
    },
    goerli: {
        timestamp: 1662129986,
        network: "goerli",
        contracts: {
            AddressBook: {
                address: "0x1B6e08713D2853e20f1F3370B9F809d3B20944Bd",
                blockNumber: 7516512,
            },
            Otoken: {
                address: "0xeb4eFc7c3f048aeF7AC80a86Ac9119432BaB093e",
                blockNumber: 7516519,
            },
            Oracle: {
                address: "0x5c26B53A6e8E50478AfEa123a3726E7188d1558B",
                blockNumber: 7516530,
            },
            MarginCalculator: {
                address: "0x008b8Dd774ac74dde38ea49ade5298282A9b107D",
                blockNumber: 7516539,
            },
            OtokenFactory: {
                address: "0x41D4Fe19c8C41Dc60638B5d12Cf32d59CFbac73a",
                blockNumber: 7516546,
            },
            Whitelist: {
                address: "0x8a97ef0A5b4AD17DbE509c2fd0F68879aa9904ad",
                blockNumber: 7516553,
            },
            MarginPool: {
                address: "0xaF520C36AB5a88f305F36D0c7Fa36483e52a402C",
                blockNumber: 7516561,
            },
            MarginVault: {
                address: "0xfD503Bc2A082a5ae82a4f6A1c97a80Dd69455788",
                blockNumber: 7516568,
            },
            Controller: {
                address: "0x06fcDD279c3A297864C93f45d34ffcDBD834f3D9",
                blockNumber: 7516577,
            },
            USDC: {
                address: "0x786A7c36d8b3acE2AE2A62c00D915C9f84eaAcB7",
                blockNumber: 0,
            },
            CurveManager: {
                address: "0x809Ce01AD915685878649E5201808af6AE01CDc4",
                blockNumber: 7516589,
            },
            CriteriaManager: {
                address: "0xD77399aEf4d4c7A54eA20eF0943b9e722dEf1339",
                blockNumber: 7516597,
            },
            PotionLiquidityPool: {
                address: "0x8a450F4C1aF53a5b41F6ec8f05036bE1F7383fEc",
                blockNumber: 7516607,
            },
            ChainLinkPricerWETH: {
                address: "0x54235B44Ebb8F8Bf1bc248339ebAF06f7829307C",
                blockNumber: 7516614,
            },
            ChainLinkPricerWBTC: {
                address: "0xb64c7dE03c87DcA86bb9d8f2a6ee00ced1a5bd5F",
                blockNumber: 7516618,
            },
            ChainLinkPricerUSDC: {
                address: "0x29D3bbB8088d3962B80ca06cE04EE43db46B2AEc",
                blockNumber: 7516622,
            },
        },
    },
    "goerli.prepare-testcomp": {
        timestamp: 1668585964,
        network: "goerli",
        contracts: {
            WETH: {
                address: "0x06BD5AB3c3272f8E210F5D5e3AC869890bD4699C",
                blockNumber: 7962549,
            },
            WBTC: {
                address: "0x4FEb584db02401c9C3Fa2EFB6097794Fc58f1f30",
                blockNumber: 7962559,
            },
            LINK: {
                address: "0xA5fE5Cd087538Bcf85baCAb570FacF95bddec1A8",
                blockNumber: 7962567,
            },
            USDC: {
                address: "0x51E857Dd537e9B9125d3025eC8051Bda4d9510d9",
                blockNumber: 7962573,
            },
        },
    },
    "kovan.demo": {
        collateralTokenAddress: "0x7e6edA50d1c833bE936492BF42C1BF376239E9e2",
        opynAddressBookAddress: "0x8812f219f507e8cfe9d2f1e790164714c5e06a73",
        potionLiquidityPoolAddress: "0xaf3DE2609F8b324122037071C2B56bFFB831C64f",
        curveManagerAddress: "0x376F8c85F43DA126A65C376F0612797695e806Da",
        criteriaManagerAddress: "0xD4aA9ED730730272572371EFB2a8f49B6D3f17Ca",
        otokenFactoryAddress: "0xb9D17Ab06e27f63d0FD75099d5874a194eE623e2",
        whitelistAddress: "0x9164eB40a1b59512F1803aB4C2d1dE4B89627A93",
        network: "kovan",
    },
    "kovan.independent": {
        collateralTokenAddress: "0x889805Cc6488818Bb8fc2A7bF66B842061A72960",
        opynAddressBookAddress: "0x1BD00a30827768b9B6fC5E4A42F424cF57d5FE99",
        potionLiquidityPoolAddress: "0x6c2d4fAe8d7DB2bAe9802dBd637A09844dc283f9",
        curveManagerAddress: "0xfE5858dfDa11ea758AFd576DCF844A49d51E4e9f",
        criteriaManagerAddress: "0xb0aA715aa80aeCe6a1e9EBB7628dc0a0E23559e6",
        marginVaultLibAddress: "0xf74C4411caff2E65435Ab494e454f49A71B4A1D6",
        otokenFactoryAddress: "0x415789d11409533410699Cf464E6bC540E85b9D9",
        whitelistAddress: "0xAcbc7baF72b1a5E97b1e3111D5b60aE6fF19cA01",
        network: "kovan.independent",
        sampleUnderlyingTokenAddress: "0x3bDC381232F72bC3c4844666c91741551268252a",
        oracleIsMock: true,
    },
    kovan: {
        collateralTokenAddress: "0x7e6edA50d1c833bE936492BF42C1BF376239E9e2",
        opynAddressBookAddress: "0x8812f219f507e8cfe9d2f1e790164714c5e06a73",
        potionLiquidityPoolAddress: "0xfaFe702Dd55217350E8CA97f2F69Dfac632D95E2",
        curveManagerAddress: "0xe6aDc4445c6b82936DC01aBaDF63387C85DD628d",
        criteriaManagerAddress: "0x09AC85e8939C43eD63CBEE442d8C70FaD5E43100",
        otokenFactoryAddress: "0xb9D17Ab06e27f63d0FD75099d5874a194eE623e2",
        whitelistAddress: "0x9164eB40a1b59512F1803aB4C2d1dE4B89627A93",
        network: "kovan",
    },
    "localhost.goerli": {
        timestamp: 1661939003,
        network: "localhost",
        contracts: {
            AddressBook: {
                address: "0x0F9fFED1201De979dBe3A1562463B99bd5e7aB36",
                blockNumber: 7502960,
            },
            Otoken: {
                address: "0x42c0A0437106284c9aE4a3A3acb0199bF148B3eb",
                blockNumber: 7502964,
            },
            Oracle: {
                address: "0x112774740cD0D7a3cA68e8F6a7b3d15650821600",
                blockNumber: 7502965,
            },
            MarginCalculator: {
                address: "0x2EFBA7923E9A32A4C239BD2f7Da0c5b0e22a909E",
                blockNumber: 7502970,
            },
            OtokenFactory: {
                address: "0x4Af81C96d19315212358f9E4a774bBcC0390B5f0",
                blockNumber: 7502975,
            },
            Whitelist: {
                address: "0xe7D757FdeB78A806A72A652ac411738DEbC1670E",
                blockNumber: 7502980,
            },
            MarginPool: {
                address: "0x1CD1a7E4158Eb444bC55AaE2b89bE7E03a4E6Ae5",
                blockNumber: 7502985,
            },
            MarginVault: {
                address: "0xf189e9EBCE19Ca3FC94427e64d4fa64D4118a180",
                blockNumber: 7502990,
            },
            Controller: {
                address: "0x9589f149FDcEe08aB054525EF8228F10CFD359C2",
                blockNumber: 7502999,
            },
            USDC: {
                address: "0x786A7c36d8b3acE2AE2A62c00D915C9f84eaAcB7",
                blockNumber: 0,
            },
            CurveManager: {
                address: "0x69dd60b4a7Bf521Dd7bFA1871FaB6C237191191d",
                blockNumber: 7503033,
            },
            CriteriaManager: {
                address: "0x7F00A7d1Cf59dCf0857aD6bDa376ecaA0662aD9C",
                blockNumber: 7503038,
            },
            PotionLiquidityPool: {
                address: "0x6b81e57848A75369cC5F13ED98c62412a40F19E7",
                blockNumber: 7503045,
            },
            ChainLinkPricerWETH: {
                address: "0x69FDC34D9deC661EB590515a5194Ce19d52B4596",
                blockNumber: 7503064,
            },
            ChainLinkPricerWBTC: {
                address: "0xF6987b595D4196A40db287ef77eEf181BB045272",
                blockNumber: 7503074,
            },
            ChainLinkPricerUSDC: {
                address: "0x93dc9AdeABcD0B36ce7F46B0e852E77B3Fd35bFE",
                blockNumber: 7503083,
            },
        },
    },
    "localhost.hedging": {
        timestamp: 1667830062,
        network: "develop",
        contracts: {
            AddressBook: {
                address: "0x5FbDB2315678afecb367f032d93F642f64180aa3",
                blockNumber: 0,
            },
            Otoken: {
                address: "0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512",
                blockNumber: 2,
            },
            Oracle: {
                address: "0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0",
                blockNumber: 3,
            },
            MarginCalculator: {
                address: "0x36b58F5C1969B7b6591D752ea6F5486D069010AB",
                blockNumber: 96,
            },
            OtokenFactory: {
                address: "0xDc64a140Aa3E981100a9becA4E685f962f0cF6C9",
                blockNumber: 5,
            },
            Whitelist: {
                address: "0x5FC8d32690cc91D4c39d9d3abcBD16989F875707",
                blockNumber: 6,
            },
            MarginPool: {
                address: "0x0165878A594ca255338adfa4d48449f69242Eb8F",
                blockNumber: 7,
            },
            MarginVault: {
                address: "0xa513E6E4b8f2a923D98304ec87F64353C4D5C853",
                blockNumber: 8,
            },
            Controller: {
                address: "0x2279B7A0a67DB372996a5FaB50D91eAA73d2eBe6",
                blockNumber: 9,
            },
            USDC: {
                address: "0x959922bE3CAee4b8Cd9a407cc3ac1C251C2007B1",
                blockNumber: 17,
            },
            CurveManager: {
                address: "0x9A9f2CCfdE556A7E9Ff0848998Aa4a0CFD8863AE",
                blockNumber: 18,
            },
            CriteriaManager: {
                address: "0x68B1D87F95878fE05B998F19b66F4baba5De1aed",
                blockNumber: 19,
            },
            PotionLiquidityPool: {
                address: "0x59b670e9fA9D0A427751Af201D676719a970857b",
                blockNumber: 22,
            },
            SampleUnderlyingToken: {
                address: "0x322813Fd9A801c5507c9de605d63CEA4f2CE6c44",
                blockNumber: 24,
            },
            MockOracle: {
                address: "0x2E2Ed0Cfd3AD2f1d34481277b3204d807Ca2F8c2",
                blockNumber: 92,
            },
        },
    },
    "localhost.independent": {
        timestamp: 1661938622,
        network: "localhost",
        contracts: {
            AddressBook: {
                address: "0xE1C86e8E80DE01b8A87344C808671D61aEE05b33",
                blockNumber: 7502579,
            },
            Otoken: {
                address: "0x12d73e63281fD1387290D75a66861B5368B4a616",
                blockNumber: 7502580,
            },
            Oracle: {
                address: "0xb8B814Fd019C00888eb34721f87aa82E3f2E1F28",
                blockNumber: 7502585,
            },
            MarginCalculator: {
                address: "0x15cAbd0536f9707d1c03b21dDdC556726D7FF136",
                blockNumber: 7502590,
            },
            OtokenFactory: {
                address: "0x58Be5Ec7F96Ef12B6d8B41611AdE2205F0AD1df7",
                blockNumber: 7502595,
            },
            Whitelist: {
                address: "0xFE0Ef5a9B7cA4FB1a327f48771cA9b4725063A64",
                blockNumber: 7502600,
            },
            MarginPool: {
                address: "0x85bdC135Bc56C3707551E9FEa8c5d7e31C82d16a",
                blockNumber: 7502605,
            },
            MarginVault: {
                address: "0x7B9985adBc2629d3e80C18397837A967399E8EAd",
                blockNumber: 7502610,
            },
            Controller: {
                address: "0x1B5AdE7927F798171F34e94a143113443ec46435",
                blockNumber: 7502619,
            },
            USDC: {
                address: "0x786A7c36d8b3acE2AE2A62c00D915C9f84eaAcB7",
                blockNumber: 0,
            },
            CurveManager: {
                address: "0x6863952619b2a5f6556F32Cb05De23CC5d080255",
                blockNumber: 7502653,
            },
            CriteriaManager: {
                address: "0xF09d18a431C56E859885B37582BD6BDd2333b54C",
                blockNumber: 7502657,
            },
            PotionLiquidityPool: {
                address: "0xe6F314143AE46b5b0594Ae7C88e6A39D36964bb7",
                blockNumber: 7502671,
            },
            ChainLinkPricerWETH: {
                address: "0xEdBe4b9b9d47B432A8e8230b97E2e1eE78d23591",
                blockNumber: 7502691,
            },
            ChainLinkPricerWBTC: {
                address: "0xe7dA9C7486556ec0061b6dac078a8569A6873088",
                blockNumber: 7502698,
            },
            ChainLinkPricerUSDC: {
                address: "0xF771D9111454D58D728d731752770B6Bf5Ad35Bf",
                blockNumber: 7502708,
            },
        },
    },
    localhost: {
        timestamp: 1661782513,
        network: "localhost",
        contracts: {
            AddressBook: {
                address: "0x5FbDB2315678afecb367f032d93F642f64180aa3",
                blockNumber: 1,
            },
            Otoken: {
                address: "0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512",
                blockNumber: 2,
            },
            Oracle: {
                address: "0x9fE46736679d2D9a65F0992F2272dE9f3c7fa6e0",
                blockNumber: 3,
            },
            MarginCalculator: {
                address: "0x7969c5eD335650692Bc04293B07F5BF2e7A673C0",
                blockNumber: 67,
            },
            OtokenFactory: {
                address: "0xDc64a140Aa3E981100a9becA4E685f962f0cF6C9",
                blockNumber: 5,
            },
            Whitelist: {
                address: "0x5FC8d32690cc91D4c39d9d3abcBD16989F875707",
                blockNumber: 6,
            },
            MarginPool: {
                address: "0x0165878A594ca255338adfa4d48449f69242Eb8F",
                blockNumber: 7,
            },
            MarginVault: {
                address: "0xa513E6E4b8f2a923D98304ec87F64353C4D5C853",
                blockNumber: 8,
            },
            Controller: {
                address: "0x8A791620dd6260079BF849Dc5567aDC3F2FdC318",
                blockNumber: 10,
            },
            PotionTestUSD: {
                address: "0x9A9f2CCfdE556A7E9Ff0848998Aa4a0CFD8863AE",
                blockNumber: 18,
            },
            USDC: {
                address: "0x8A791620dd6260079BF849Dc5567aDC3F2FdC318",
                blockNumber: 10,
            },
            CurveManager: {
                address: "0x68B1D87F95878fE05B998F19b66F4baba5De1aed",
                blockNumber: 19,
            },
            CriteriaManager: {
                address: "0x3Aa5ebB10DC797CAC828524e59A333d0A371443c",
                blockNumber: 20,
            },
            PotionLiquidityPool: {
                address: "0x4ed7c70F96B99c776995fB64377f0d4aB3B0e1C1",
                blockNumber: 23,
            },
            SampleUnderlyingToken: {
                address: "0xa85233C63b9Ee964Add6F2cffe00Fd84eb32338f",
                blockNumber: 25,
            },
            MockOracle: {
                address: "0xb7278A61aa25c888815aFC32Ad3cC52fF24fE575",
                blockNumber: 63,
            },
            WETH: {
                address: "0xa513E6E4b8f2a923D98304ec87F64353C4D5C853",
                blockNumber: 8,
            },
            WBTC: {
                address: "0x2279B7A0a67DB372996a5FaB50D91eAA73d2eBe6",
                blockNumber: 9,
            },
            ChainlinkAggregatorUSDC: {
                address: "0x610178dA211FEF7D417bC0e6FeD39F05609AD788",
                blockNumber: 11,
            },
        },
    },
    "localhost.prepare-testcomp": {
        timestamp: 1668585408,
        network: "develop",
        contracts: {
            WETH: {
                address: "0xDc64a140Aa3E981100a9becA4E685f962f0cF6C9",
                blockNumber: 5,
            },
            WBTC: {
                address: "0x5FC8d32690cc91D4c39d9d3abcBD16989F875707",
                blockNumber: 6,
            },
            LINK: {
                address: "0x0165878A594ca255338adfa4d48449f69242Eb8F",
                blockNumber: 7,
            },
            USDC: {
                address: "0xa513E6E4b8f2a923D98304ec87F64353C4D5C853",
                blockNumber: 8,
            },
        },
    },
    "ply-mumbai.prepare-testcomp": {
        timestamp: 1668698191,
        network: "ply-mumbai",
        contracts: {
            WETH: {
                address: "0x798e9C98e24faBcCf04cF6d31381B1CFC75CBe14",
                blockNumber: 29200200,
            },
            WBTC: {
                address: "0x9649071fb3875b68C88c60b172e2F6ADa5717634",
                blockNumber: 29200208,
            },
            LINK: {
                address: "0xBB2bc2c224139512a7525a83955567FD2C3a0c1F",
                blockNumber: 29200216,
            },
            USDC: {
                address: "0x45902f8c0a64A19ff849DAD5277Af72F68C746D6",
                blockNumber: 29200224,
            },
        },
    },
    "ply-mumbai.testcomp": {
        timestamp: 1668700526,
        network: "ply-mumbai",
        contracts: {
            AddressBook: {
                address: "0xF55277d2608C69DE9A7904c8318C497f62460ef2",
                blockNumber: 0,
            },
            Otoken: {
                address: "0x5920Cc316a732106c2A79674170881f3Fcd90A8b",
                blockNumber: 29200328,
            },
            Oracle: {
                address: "0x14b101c16132377b6476Fb7228C21420C7fdd7DF",
                blockNumber: 29200336,
            },
            MarginCalculator: {
                address: "0x94a38dBE7104a69E752466Ed48Dc167c96212AEE",
                blockNumber: 29200343,
            },
            OtokenFactory: {
                address: "0x543dC5Df16d8A47E9aF1669D773D2544131557ea",
                blockNumber: 29200350,
            },
            Whitelist: {
                address: "0x7A790CC1f3baB337344088Ed2A8AB6513714529f",
                blockNumber: 29200357,
            },
            MarginPool: {
                address: "0xe22A6151ED3d442aaa0531342fc43317D0676dDC",
                blockNumber: 29200365,
            },
            MarginVault: {
                address: "0x104f3154b11A0D3C6Ab094ec26d7B9DCf15bdD26",
                blockNumber: 29200372,
            },
            Controller: {
                address: "0xD82DED161c6Bfcf0DdCc3D63a846612ad25cbc73",
                blockNumber: 29200402,
            },
            USDC: {
                address: "0x45902f8c0a64A19ff849DAD5277Af72F68C746D6",
                blockNumber: 0,
            },
            CurveManager: {
                address: "0xfD503Bc2A082a5ae82a4f6A1c97a80Dd69455788",
                blockNumber: 29200460,
            },
            CriteriaManager: {
                address: "0x3160bf63bCC80d384Df575163A7E3AfCE29f14dB",
                blockNumber: 29200471,
            },
            PotionLiquidityPool: {
                address: "0x7e26484f6d9383a3c16DEaab8C7D1Fc7b0e88a66",
                blockNumber: 29200484,
            },
            ChainLinkPricerWETH: {
                address: "0xB7125B8178B60b3F9cF6f9C2cDa03119c5F4b2D7",
                blockNumber: 29200499,
            },
            ChainLinkPricerWBTC: {
                address: "0xD77399aEf4d4c7A54eA20eF0943b9e722dEf1339",
                blockNumber: 29200509,
            },
            ChainLinkPricerLINK: {
                address: "0xaadD9De5c06AA107501413228F898C1E6A6D770e",
                blockNumber: 29200521,
            },
            ChainLinkPricerUSDC: {
                address: "0xAF112F090e6132c23D7275c60a96CF48672f8dd1",
                blockNumber: 29200532,
            },
        },
    },
};
