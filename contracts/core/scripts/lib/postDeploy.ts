...
class UnsettledOtoken {
    public constructor(
        ...
        public contributors: CounterpartyDetails[] = [],
    ) {}
}
    ...
    private async _push(res: PostDeployActionSingleResult): Promise<void> {
        if (res) {
            ...
            if (PostDeployActionsResults.isOtoken(res)) {
                this.unsettledOtokens[res.address] = new UnsettledOtoken(
                    res.address,
                    (await res.expiryTimestamp()).toNumber(),
                );
            }
            ...
            if (res instanceof PurchaseParams) {
                ...
                this.unsettledOtokens[otokenAddress].contributors = this.unsettledOtokens[
                    otokenAddress
                ].contributors.concat(res.sellers);
            }
