...
export class InitializeMockOracle implements PostDeployAction {
    ...
    async executePostDeployAction(
        ...
        trx = await mockOracle.setRealTimePrice(
            depl.sampleUnderlyingTokenAddress,
            scaleNum(this.initialEthPriceInDollars),
        );
        await trx.wait();
...
export class UpdateMockOraclePrices implements PostDeployAction {
    ...
    async executePostDeployAction(
        ...
        if (otokens.length > 0) {
            printProgress &&
                console.log(
                    `Setting mock oracle price to $${
                        this.priceInDollars
                    }, including expiry prices for otokens: ${otokens.map(o => o.address).join()}`,
                );
        } else {
