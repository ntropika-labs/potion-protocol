    ...
    describe("powerDecimal", () => {
        ...
        powerDecimalTestCases.forEach(tc => {
            it(`powerDecimal(${tc.in_base_number},${tc.in_exponent_number})`, async () => {
                const res = new Int59x18(
                    await curveManager.powerDecimal(tc.in_base_59x18.value, tc.in_exponent_59x18.value),
                );
        ...
        powerDecimalFailureCases.forEach(tc => {
            it(`out of bounds: powerDecimal(${tc.in_base_number},${tc.in_exponent_number})`, async () => {
                await expect(
                    curveManager.powerDecimal(tc.in_base_59x18.value, tc.in_exponent_59x18.value),
                ).to.be.revertedWith("powerDecimal: base must be >= 0");
            });
        });
    });
    ...
    describe("hyperbolicCurves", () => {
        ...
        curve1TestCases.forEach(tc => {
            it(`curve1(${tc.x_number})`, async () => {
                const res = new Int59x18(
                    await curveManager.hyperbolicCurve(tc.curve.asSolidityStruct(), tc.x_59x18.value),
                );
        ...
        myTests.forEach(tc => {
            it(`curve{${tc.curve.a_number},${tc.curve.b_number},${tc.curve.c_number},${tc.curve.d_number}}(${tc.x_number})`, async () => {
                ...
                if (answDecimal.toNumber() == 0) {
                    assert(
                        resDecimal.toNumber() == 0,
                        `Zero expectation yields non-zero result of ${resDecimal.toString()}`,
                    );
                } else {
                    const percentRatio = answDecimal.gt(resDecimal)
                        ? answDecimal.div(resDecimal)
                        : resDecimal.div(answDecimal);
