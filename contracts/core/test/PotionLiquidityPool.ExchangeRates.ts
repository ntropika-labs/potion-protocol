...
describe("PotionLiquidityPool (Exchange Rates)", function () {
    ...
    before("set up contracts", async () => {
        ...
        ({ addressBook, potionLiquidityPool, usdc, weth, oracle, otokenFactory } = testContracts);
        ...
        whitelist = (await MockWhitelistFactory.deploy()) as MockWhitelistModule;
        ...
        for (let j = 0; j < OTOKEN_QUANTITIES.length; j++) {
            ...
            for (let i = 0; i < numOptions; i++) {
                it(`strike=${strikePricesInDollars[i]},qty=${qty}`, async () => {
                    ...
                    const expectedCollateralInUsdWith8Decimals = strikePriceWith8Decimals
                        .mul(qty)
                        .div(OTOKEN_WEI_PER_OTOKEN);
                    ...
                    if (collateralToken.address === strikeToken.address) {
                        ...
                        expect(await potionLiquidityPool.collateralNeededForPuts(otoken.address, qty)).to.equal(
                            expectedVal,
                        );
                    } else {
                        ...
                        const strikePriceInNumberOfNickels =
                            BigNumber.from(NICKELS_PER_DOLLAR).mul(strikePriceInDollars);
                        const strikePriceInNicklecoinWei = shift(
                            strikePriceInNumberOfNickels,
                            await nickleCoin.decimals(),
                        );

                        // When multiplying by otoken quantity in wei, we divide by OTOKEN_WEI_PER_OTOKEN to use otoken quantity in human units
                        const totalCollateralInNickelcoinWei = strikePriceInNicklecoinWei
                            .mul(qty)
                            .div(OTOKEN_WEI_PER_OTOKEN);
