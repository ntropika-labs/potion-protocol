...
describe("PotionLiquidityPool - Config tests", function () {
    ...
    describe("Deposit and config tests", () => {
        ...
        it("deposit", async () => {
            ...
            // pool balances
            expect(await usdc.balanceOf(potionLiquidityPool.address)).to.equal(
                depositAmount2.mul(4).add(depositAmount1),
            );
            expect(await weth.balanceOf(potionLiquidityPool.address)).to.equal(0);
        ...
        it("tests sumOfAllLockedBalances", async () => {
            ...
            const lockedBalanceAfterSettlement = await potionLiquidityPool
                .connect(potionBuyer1)
        ...
        it("tests cannot buy with premium", async () => {
            ...
            const newOrderSize = newLiquidityAndPremium
                .div(createTokenAmount(lowerStrikeInDollars, usdcDecimals))
                .toNumber();
    ...
    describe("Withdraw tests", async () => {
        it("withdraws", async () => {
            ...
            expect(await potionLiquidityPool.sumOfAllUnlockedBalances()).to.equal(
                depositAmount1.sub(halfDepositAmount),
            );
        });
