...
describe("PotionLiquidityPool - Upgrades", function () {
    ...
    before("set up contracts", async () => {
        ({ addressBook, potionLiquidityPool, usdc, weth, oracle, otokenFactory, curveManager, criteriaManager } =
    ...
    describe("with an initial contract deployed", () => {
        ...
        it("functions fail if not defined in original contract", async () => {
            ...
            /*
              ...
              Once the hardhat version is updated to include the fix, the below lines of code must be used instead of the ones right after this
              comment:
            ...
            */
            await expect(upgradedInterface.upgradeTestVal()).to.be.reverted;
            await expect(upgradedInterface.setUpgradeTestVal(7)).to.be.reverted;
        });
        ...
        describe("after contract upgrade", () => {
            ...
            it("internal balances as expected", async () => {
                expect(await potionLiquidityPool.lpLockedAmount(potionLp1.address, poolId1), "LpLockedAmount").to.equal(
                    0,
                );
                expect(await potionLiquidityPool.lpTotalAmount(potionLp1.address, poolId1), "LpTotalAmount").to.equal(
                    depositAmount,
            ...
            describe("after LP withdraws the other half of their capital", () => {
                ...
                it("internal balances as expected", async () => {
                    expect(
                        await potionLiquidityPool.lpLockedAmount(potionLp1.address, poolId1),
                        "LpLockedAmount",
                    ).to.equal(0);
                    expect(
                        await potionLiquidityPool.lpTotalAmount(potionLp1.address, poolId1),
                        "LpTotalAmount",
                    ).to.equal(0);
                });
        ...
        describe("with paused contract", () => {
            ...
            it("buying otokens disabled", async () => {
                const counterparties = [
                    new CounterpartyDetails(potionLp1, poolId1, curve1, criteria1, BigNumber.from(1000)),
                ];
                await expect(
                    potionLiquidityPool
                        .connect(potionLp1)
            ...
            describe("after contract upgrade", () => {
                ...
                it("deposits disabled", async () => {
                    ...
                    await expect(
                        potionLiquidityPool.depositAndConfigurePool(
                            poolId1,
                            depositAmount,
                            curveHash1,
                            criteriaSetHash,
                ...
                it("pool config disabled", async () => {
                    ...
                    await expect(
                        potionLiquidityPool.connect(potionLp1).setCurve(poolId1, curveHash1),
                    ).to.be.revertedWith(`Pausable: paused`);
                });
