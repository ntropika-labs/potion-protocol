...
import { getChainConfig, Networks } from "../hardhat/hardhat.helpers";
import type { NetworksType } from "../hardhat/hardhat.helpers";
...
async function runNode(network: NetworksType, blocktime: number | undefined) {
  ...
  ganacheArgs.push(
    ...["-f", config.url, "--chain.chainId", String(config.chainId)]
  );
...
async function runLocalNode() {
  const ganacheArgs = [
    "-m",
    String(process.env.DEPLOYER_MNEMONIC),
    "--hostname",
    "0.0.0.0",
  ];
