...
export class HyperbolicCurve {
  ...
  toArray(): (string | boolean | BigNumberish)[] {
    ...
    assert(
      values.length === HyperbolicCurve.solidityTypes().length,
      "curve: # of solidity types != # of members"
    );
    return values;
  }
...
export class CurveCriteria {
  ...
  toArray(): (string | boolean | BigNumberish)[] {
    const values = [
      ...
      this.strikeAsset,
      this.isPut,
      this.maxStrikePercent,
      this.maxDurationInDays,
    ];

    // Check for an out of date solidityTypes()
    assert(
      values.length === CurveCriteria.solidityTypes().length,
...
export class CriteriaSet {
  ...
  toKeccak256(): string {
    return utils.solidityKeccak256(CriteriaSet.solidityTypes(), [
      this.toArray(),
    ]);
  }
}
